...
def prepare_oracle_table_5g(oracle_con, oracle_cur, frequency_type, field_mapping_dic, base_mapping_700_dic, base_mapping_redzone_700_dic, base_mapping_2600_dic, base_mapping_redzone_2600_dic, drop_param=True, baseline_label_dic={}):
	for group_param in field_mapping_dic:
		...
		if (ZTE_TABLE_PREFIX + "_" + frequency_type) not in CREATED_TABLE:
			...
			try:
				...
				if (len(base_mapping_redzone_700_dic) != 0):
					ran_baseline_oracle.push(oracle_cur, table_name, base_mapping_redzone_700_dic[group_param])
...
def prepare_oracle_table_4g(oracle_con, oracle_cur, frequency_type, field_mapping_dic, base_mapping_dic, red_mapping_dic, base_mapping_700_dic, base_mapping_redzone_700_dic, base_mapping_2600_dic, base_mapping_redzone_2600_dic, base_mapping_700_anchor_dic, base_mapping_900_anchor_dic, base_mapping_1800_anchor_dic, base_mapping_2100_anchor_dic, base_mapping_2600_anchor_dic,  drop_param=True, baseline_label_dic={}):
	for group_param in field_mapping_dic:
		table_name = naming_helper.get_table_name(BASELINE_TABLE_PREFIX.format(ZTE_TABLE_PREFIX), frequency_type, group_param)
		...
		if (ZTE_TABLE_PREFIX + "_" + frequency_type) not in CREATED_TABLE:
			...
			try:
				...
				if len(base_mapping_redzone_700_dic) != 0:
					ran_baseline_oracle.push(oracle_cur, table_name, base_mapping_redzone_700_dic[group_param])
				if len(base_mapping_700_anchor_dic) != 0:
					ran_baseline_oracle.push(oracle_cur, table_name, base_mapping_700_anchor_dic[group_param])
							...
							if parameter_group.upper() == 'Sctp'.upper():
								...
								for mo in mo_group_collection:
									...
									if level_type == 'CELL Level':
										...
										elif parameter_group.upper() == 'NRSectorCarrier'.upper() or parameter_group.upper() == 'SectorFunction'.upper() or parameter_group.upper() == 'GNBCUUPFunction'.upper() or parameter_group.upper() == "ENDCPDCPStatusCfg".upper() or parameter_group.upper() == 'ZTE_5G_X2SCPOLICY'.upper() or parameter_group.upper() == 'CarrierESPolicy'.upper():
